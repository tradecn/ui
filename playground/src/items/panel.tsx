...
function Book({ id, group, symbol }: { id: string; group: LinkGroup; symbol: string }) {
  ...
  const popout = usePopout({ title: `${id} ${link.symbol ?? ""}`, width: 360, height: 260 })
  ...
  return (
    <PanelPopout
      popout={popout}
      placeholder={
        <div className="flex h-44 items-center justify-center rounded-md border border-dashed border-border text-muted-foreground">
    ...
    >
      <Panel kind="book" className="h-44">
        <PanelHeader>
...
function States() {
  ...
  return (
    <Panel kind="states" className="h-44" active={state === "active" ? true : undefined} dragTarget={state === "dragTarget"} error={state === "error"}>
      <PanelHeader>
