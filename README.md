...
```bash
npx shadcn@latest add tradecn/ui/data-grid#v1.4.7
```
...
That URL is the latest release. Put the tag in it to pin: `https://tradecn.dev/r/v1.4.7/{name}.json`. Same files either way.
...
```bash
npx shadcn@latest add tradecn/ui/data-grid#v1.4.7 --diff   # a tradecn change
