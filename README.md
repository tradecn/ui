...
```bash
npx shadcn@latest add tradecn/ui/data-grid#v1.3.1
```
...
That URL is the latest release. Put the tag in it to pin: `https://tradecn.dev/r/v1.3.1/{name}.json`. Same files either way.
...
```bash
npx shadcn@latest add tradecn/ui/data-grid#v1.3.1 --diff   # a tradecn change
