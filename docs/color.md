...
For a category, one line among several on a chart, the eight of Okabe and Ito are the palette, and the chart tokens `chart-1` to `chart-8` carry them: the items' own values follow their order, orange, sky blue, bluish green, yellow, blue, vermilion, reddish purple, and the page's own foreground for black, each at the lightness its side needs; a theme sets all eight in its own palette, and a shadcn project keeps the five it already has. [`price-chart`](price-chart.md) draws its overlays with them beside a legend that names each line, so no line is known by its color alone. For a quantity rather than a category, a heat map or a depth ladder, use a perceptually uniform ramp such as viridis, which was drawn to read for people with color blindness and to print in grayscale ([the viridis vignette](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html)); a diverging ramp for a signed quantity keeps a light middle and never runs red to green. tradecn ships no heat map yet, so the ramp is advice, not a token.
