...
For a category, one line among several on a chart, the eight of Okabe and Ito are the palette: the chart tokens `chart-1` to `chart-8` follow their order, orange, sky blue, bluish green, yellow, blue, vermilion, reddish purple, and the page's own foreground for black, each at the lightness its side needs, and [`price-chart`](price-chart.md) draws its overlays with them beside a legend that names each line. For a quantity rather than a category, a heat map or a depth ladder, use a perceptually uniform ramp such as viridis, which was drawn to read for people with color blindness and to print in grayscale ([the viridis vignette](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html)); a diverging ramp for a signed quantity keeps a light middle and never runs red to green. tradecn ships no heat map yet, so the ramp is advice, not a token.
