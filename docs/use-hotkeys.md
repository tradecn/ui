...
`npx shadcn add tradecn/ui/use-hotkeys` puts `use-hotkeys.tsx` in your `hooks` alias and `hotkeys.ts` in your `lib` alias. No dependencies beyond React, and no shadcn component required. `formatKeys` returns key caps shaped for shadcn's `kbd` if you have it.
