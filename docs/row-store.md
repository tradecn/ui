...
`npx shadcn add tradecn/ui/row-store` puts `row-store.ts` in your `lib` alias and `use-row-store.ts` in your `hooks` alias. No dependencies beyond React.

The store is the seam between your feed and the components. You own the feed; the store owns the rows; components subscribe to one row each.
...
`useRow` re-renders for a change to its row and for nothing else. Every notification fired inside one `applyDeltas` lands in one React commit, so a batch that touches two thousand rows is one render pass over those rows, not two thousand.
...
`useStoreMeta(store)` gives `version`, `size`, `lane`, cumulative `dropped`, last `seq`, `gap`, `lastBatchAt`, and `producedAt`, updated once per batch. Map it to a `FeedDescriptor` for the feed-health strip.
