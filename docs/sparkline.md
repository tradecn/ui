...
`npx shadcn add tradecn/ui/sparkline` puts `sparkline.tsx` in your `ui` alias, `use-flash.ts` in your `hooks` alias, and `sparkline-geometry.ts` in your `lib` alias, and adds the market tokens if you do not have them: `up`, `down`, `flat`, and their `-soft` variants. Dependency: `cn`. No shadcn component required.
