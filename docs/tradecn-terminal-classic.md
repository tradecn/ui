...
`npx shadcn add tradecn/ui/tradecn-terminal-classic` is `tradecn-terminal` with green for up and red for down. It installs no files and rewrites the variables in your stylesheet. Read `docs/tradecn-terminal.md` first: everything there about overwriting, `--diff`, both modes carrying one palette, and how the colors are checked is true of this one too.

```bash
npx shadcn@latest add tradecn/ui/tradecn-terminal-classic --diff   # look first
npx shadcn@latest add tradecn/ui/tradecn-terminal-classic
```
