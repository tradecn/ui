...
`npx shadcn add tradecn/ui/ticket` puts `ticket.tsx` in your `components` alias (not `components/ui`: this is the registry's first block), `use-flash.ts` and `use-hotkeys.tsx` in your `hooks` alias, `format.ts` and `hotkeys.ts` in your `lib` alias, and brings your own `button`, `button-group`, `field`, `input`, `input-group`, `kbd`, and `native-select` if you do not have them. It adds the `up` and `down` tokens and their `-soft` variants. Dependency: `cn`. If you already installed `format`, `flash-cell`, or `use-hotkeys`, the shared files are byte-identical and nothing of yours changes.
...
It does not send anything, and it does not decide anything about the order after your `run` returns. Three things are the server's, and this item holds the line on them the same way `blotter` does:
