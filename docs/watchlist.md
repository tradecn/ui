...
`npx shadcn add tradecn/ui/watchlist` puts `watchlist.tsx` and `data-grid.tsx` in your `ui` alias, brings `row-store`, `use-row-store`, `use-flash`, and `format` with them, and your own `input`, `button`, `context-menu`, `checkbox`, and `dropdown-menu` if you do not have them. Dependencies: `cn`, `@tanstack/react-virtual`. If you already installed `data-grid`, the shared files are byte-identical and nothing of yours changes.
...
It is the data grid in its `watchlist` preset (22 px rows, single select, fill flash) with a set of columns, a field that adds, and three ways to remove. Every `DataGrid` prop passes through except `preset`. Read `docs/data-grid.md` for sorting, the reorder hold, column state, and the keyboard.
