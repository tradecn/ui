...
`npx shadcn add tradecn/ui/flash-cell` puts `flash-cell.tsx` in your `ui` alias and `use-flash.ts` in your `hooks` alias, and adds the market tokens to your stylesheet: `up`, `down`, `flat`, and their `-soft` variants, usable as `text-up`, `bg-down-soft`, and so on. Dependency: `cn`.
...
The hook is `useFlash(ref, value, options)` if you would rather flash your own element.
