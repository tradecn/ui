...
- The pages are `scripts/build-site.ts` over `site/index.html`, `site/404.html`, and `site/docs.html`, filled from the tag's `registry.json`, `version.txt`, and `docs/*.md`: the landing page, `/docs/` and one page per doc at `/docs/<name>/`, rendered with `marked`. The palette is the `tradecn-terminal` theme's `cssVars`, read from `main`'s `registry.json`, since a tag from before the theme existed has none. The docs are the tag's, so `/docs/` describes what `/r/` serves.
